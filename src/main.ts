...
export default class ActionsPlugin extends Plugin {
			...
			switch (action.hook) {
				...
				case 'manual': {
					this.addCommand({
						...
						name: action.name,
						callback: () => (this.execute(action.id)),
					});
				...
				case 'createFile': {
					this.registerEvent(this.app.vault.on('create', () => (this.execute(action.id))));
				...
				case 'modifyFile': {
					this.registerEvent(this.app.vault.on('modify', () => (this.execute(action.id))));
				...
				case 'deleteFile': {
					this.registerEvent(this.app.vault.on('delete', () => (this.execute(action.id))));
				...
				case 'renameFile': {
					this.registerEvent(this.app.vault.on('rename', () => (this.execute(action.id))));
	 ...
	 * @see handleShell Executes in a plain shell environment.
	 */
	public async execute(id: string, options?: Partial<ExecutionOptions>): Promise<void> {
	...
	private handleJS(id: string, code: string): void {
		...
		try {
			...
			console.error(`failed to execute \`js\` action '${id}', gave the following error ${err}`);
		}
	...
	private handleShell(id: string, code: string): void {
		...
		exec(code, (error, stdout, stderr) => {
			if (error) {
				console.error(`failed executing \`shell\` action '${id}', ${error}`);
				return;
			}
